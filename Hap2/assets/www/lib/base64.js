...
var AQUTILB64 = {
    ...
    encode: function (s) {
        ...
        if (AQUTILB64.ie) {
            ...
            while (++position < len) {
                ...
                result.push(AQUTILB64.alphabet[enc[0]], AQUTILB64.alphabet[enc[1]], AQUTILB64.alphabet[enc[2]], AQUTILB64.alphabet[enc[3]]);
            }
            return result.join('');
        } else {
            result = '';
            while (++position < len) {
                ...
                result += AQUTILB64.alphabet[enc[0]] + AQUTILB64.alphabet[enc[1]] + AQUTILB64.alphabet[enc[2]] + AQUTILB64.alphabet[enc[3]];
            }
            return result;
        }
    },
    decode: function (s) {
        var buffer = AQUTILB64.fromUtf8(s),
            position = 0,
            len = buffer.length;
        if (AQUTILB64.ieo) {
            result = [];
    ...
    fromUtf8: function (s) {
        var position = -1,
            ...
            enc = [, , , ];
        if (!AQUTILB64.lookup) {
            len = AQUTILB64.alphabet.length;
            AQUTILB64.lookup = {};
            while (++position < len)
            AQUTILB64.lookup[AQUTILB64.alphabet[position]] = position;
            position = -1;
        }
        len = s.length;
        while (position < len) {
            enc[0] = AQUTILB64.lookup[s.charAt(++position)];
            enc[1] = AQUTILB64.lookup[s.charAt(++position)];
            buffer.push((enc[0] << 2) | (enc[1] >> 4));
            enc[2] = AQUTILB64.lookup[s.charAt(++position)];
            if (enc[2] == 64) break;
            buffer.push(((enc[1] & 15) << 4) | (enc[2] >> 2));
            enc[3] = AQUTILB64.lookup[s.charAt(++position)];
            if (enc[3] == 64) break;
