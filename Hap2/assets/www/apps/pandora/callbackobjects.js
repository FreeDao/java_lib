...
PANDORA.CallbackObj = {
	...
	onUpdateNotice : function(payload){
		...
		switch (notice){
			case "PNDR_NOTICE_ERROR_SESSION_ALREADY_STARTED":
				//TODO : Should check the status first : onGetStatus
				//PANDORA.Model.getTrackInfo();
				PANDORA.Model.getExtendTrackInfo();
	...
	onReturnTrackAlbumArtSegment : function(payload){
		...
		console.log( "data in onReturnTrackAlbumArtSegment:... " + JSON.stringify(data, undefined, 2));
	...
	onUpdateTrackAlbumArt : function(payload){
		...
		console.log( "data in onUpdateTrackAlbumArt:... " + JSON.stringify(data, undefined, 2));
