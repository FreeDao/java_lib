...
PANDORA.Model = {
	...
	sessionStart : function(apiVersion, cId, albumArtDimension, stationArtDimensionn) {
		...
		if (!albumArtDimension)
			albumArtDimension = PANDORA.CONFIG.albumArtDimension;
		if (!stationArtDimensionn)
			stationArtDimensionn = PANDORA.CONFIG.stationArtDimension;
        var payload = new SessionStartPayload();
        payload.setApiVersion(apiVersion);
        payload.setAccessoryId(cId);
        payload.setAlbumArtSize(albumArtDimension);
        payload.setStationArtDimension(stationArtDimensionn);
	...
	*/
	...
	sendData : function(payload){
        ...
        var data = AQ.app.hexToBase64(requestData);
