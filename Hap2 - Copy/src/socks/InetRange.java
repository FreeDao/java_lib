...
public class InetRange implements Cloneable{
    ...
    public synchronized boolean add(String s){
      ...
      if(s.charAt(s.length()-1) == '.'){
         ...
         entry = new Object[] {s,null,Long.valueOf(from),Long.valueOf(to)};
    ...
    public synchronized void add(InetAddress ip){
       long from, to;
       from = to = ip2long(ip);
       all.addElement(new Object[]{ip.getHostName(),ip,Long.valueOf(from),Long.valueOf(to)});
    }
    ...
    public synchronized void add(InetAddress from,InetAddress to){
       all.addElement(new Object[]{from.getHostAddress()+":"+to.getHostAddress()
                            ,null,Long.valueOf(ip2long(from)),
                            Long.valueOf(ip2long(to))});
    }
...
/////////////////
    ...
    private void resolve(Object[] entry){
       ...
       if(ip >= 0){
         entry[2] = entry[3] = Long.valueOf(ip);
       }else{
    ...
    private void resolve(Object[] entry,String from,String to){
       ...
       if((f=host2long(from))>= 0 && (t=host2long(to)) >= 0){
         entry[2] = Long.valueOf(f);
         entry[3] = Long.valueOf(t);
       }else{
   ...
   public void run(){
     try{
        if(from == null){
          ...
          entry[1] = ip;
          Long l = Long.valueOf(InetRange.ip2long(ip));
          entry[2] = entry[3] = l;
        }else{
          InetAddress f = InetAddress.getByName(from);
          InetAddress t = InetAddress.getByName(to);
          entry[2] = Long.valueOf(InetRange.ip2long(f));
          entry[3] = Long.valueOf(InetRange.ip2long(t));
