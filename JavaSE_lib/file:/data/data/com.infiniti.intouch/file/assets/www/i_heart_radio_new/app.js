...
AQ.app.loadScript(["constants.js", "generator.js", "model.js", "cache.js", "util.js"], function() {
	try {
		APP = new iHeartRadio();
		APP.init();
	} catch(err) {
 ...
 */
 function iHeartRadio() {
	...
	this.requestCallbackDispture = function(data){		
		...
		switch (para.name){
			...
			case "now_playing":
				...
				requestCallback = this.displayNowPlaying;
			break;
			case "my_stations":
				//this.displayMyStationSubmenu(); //TODO : 
			...
			case "my_stations_orderby":
				...
				requestCallback = this.displayMyStationList;  //TODO : 
			break;
			*/
	...
	this.displayNowPlaying = function(data){
		if(data.errors != null){
			console.log("error=========");
		}else{
			if(!data.state.isPlaying){
				//TODO : show notication that no station was playing
				console.log("===========no station was playing=========");
				return;
			}else{
				//TODO : show player
			}
	...
	this.displayMyStationList = function(data) {
		...
		//
		iHR.Display.myStationList(myStations);
	};
