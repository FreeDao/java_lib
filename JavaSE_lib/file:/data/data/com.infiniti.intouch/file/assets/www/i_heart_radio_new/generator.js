 ...
 */
 iHR.Display = {
	...
	myStationList: function(data) {
 		this._historySave({page:'myStationList', value:data});
 		var template_data = $.extend(true, {}, iHR.STATE.my_station_list);
 		for(var i in data){
 			template_data.template_content.list.push({
 				"text" : data[i].name,
 				"action" : "initPlayerData",
 				"value" : JSON.stringify(data[i]) //token
 			});
 		}
	...
	featuredArtists: function(data) {
 	...
 	showCustomStationPlayer: function(data) {
		...
		template_data.template_content.buttons["4"] = {
			"image" :  (data.isliked==1) ? iHR.IMAGES.button['4_liked'] : iHR.IMAGES.button['4_like'] ,
			"action" :  (data.isliked==1) ? "":"asynchronousEventPostDispture",
			"value" :  "THUMBSUP"
		};
		template_data.template_content.buttons["5"] = {
			"image" : (data.isliked==2) ? iHR.IMAGES.button['5_disliked']:iHR.IMAGES.button['5_dislike'],
		...
		if(data.trackInfo!=null && typeof(data.trackInfo.title)!="undefined"){
 	...
 	showLiveStationPlayer: function(data) {
		...
		if(data.trackInfo!=null && typeof(data.trackInfo.artistName)!="undefined"){
